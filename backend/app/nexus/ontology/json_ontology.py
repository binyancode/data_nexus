    ...
    def _build(self) -> None:
        ...
        for r in g.get("relations", []):
            ...
            if not rid:
                continue
            self._add(Concept(
                id=rid, kind=ConceptKind.relation, name=r.get("name") or rid,
                semantics=r.get("semantics"), synonyms=_syn(r.get("synonyms")),
                attrs={"from_entity": r.get("from_entity"), "from_key": r.get("from_key"),
                       "to_entity": r.get("to_entity"), "to_key": r.get("to_key")},
            ))
        ...
        for m in g.get("metrics", []):
            ...
            if not mid:
                continue
            self._add(Concept(
                id=mid, kind=ConceptKind.metric, name=m.get("name") or mid,
                semantics=m.get("semantics"), synonyms=_syn(m.get("synonyms")),
                attrs={"expr": m.get("expr")},
            ))
